...
| E2E | `cd e2e && npm test` | Playwright; 아래 **테스트 실행 방법** 참고 |
...
로컬에서 **MySQL·Redis·Kafka 없이** 돌아가는 것은 백엔드·프론트 단위 테스트입니다. E2E는 UI(기본 `http://127.0.0.1:5173`)와 API가 함께 떠 있어야 합니다.

### 백엔드 (JUnit / Spring Boot Test)
...
- **프로파일**: `src/test/resources/application-test.yml` — H2 인메모리, Flyway 비활성 등.
- **캐시 없이 전부 다시 실행**: `./gradlew test --rerun-tasks`
...
cd frontend
npm install   # 최초 1회
npm run test  # Vitest run (CI용과 동일하게 한 번 실행)
```

- **타입 검사만**: `npx tsc --noEmit`
- **워치 모드**(선택): `npx vitest` (패키지 스크립트에 없으면 위 명령 직접 실행)

### E2E (Playwright)

브라우저가 **Vite 개발 서버**에 붙고, Vite가 `/api`를 백엔드로 넘깁니다. 보통 **개발용 Compose**를 켠 뒤 실행합니다.
...
4. **옵션**
   ...
   - **마지막 실행 HTML 리포트**: `npm run report` (= `playwright show-report`)
   - **다른 UI URL**: `BASE_URL=http://localhost:5173 npm test` (기본값은 `http://127.0.0.1:5173`)
   - **관리자 시나리오**(`admin.spec.ts`): 부트스트랩 ADMIN 계정 사용. 기본은 dev Compose와 동일하게 `admin@board.local` / `ChangeMe_Admin_1`. 바꿀 때는 `E2E_ADMIN_EMAIL`, `E2E_ADMIN_PASSWORD` 환경 변수.

5. **스킵 조건**: 각 spec이 `GET /api/v1/health`(프록시 기준 UI origin)를 호출해 **200이 아니면 해당 테스트 파일 전체를 스킵**합니다. E2E가 전부 skipped면 Compose·포트·프록시를 확인하세요.

6. **스펙 위치**: `e2e/tests/*.spec.ts` (`board`, `auth-and-profile`, `posts-interactions`, `comments`, `pages`, `admin` 등)
...
- [x] REST: `POST/GET/PUT/DELETE /api/v1/posts`, `GET /api/v1/posts?size&cursor` — 키셋(커서) 페이지네이션, `createdAt desc, id desc` (최대 100건/요청)
...
| 메서드 | 경로 | 설명 |
|--------|------|------|
| GET | `/api/v1/posts?size=20` | 첫 목록 페이지 (`PostCursorPageResponse`: `content`, `nextCursor`, `size`, `hasNext`) |
| GET | `/api/v1/posts?size=20&cursor=…` | 다음 페이지 (`cursor` = 직전 응답 `nextCursor`) |
...
**다음(로드맵 후속)**: 본문 길이·검색, Snowflake datacenter/worker 설정 외부화.
...
로드맵 README · 위 단계별 **후속** 항목을 한곳에 모은 체크리스트입니다. 착수할 때마다 항목을 고치고, 완료되면 해당 단계 본문에 반영합니다.

### 기능·품질 (1차 완료 도메인의 후속)

| 우선(참고) | 내용 |
|------------|------|
| 높음 | **게시글 검색·필터** (제목/본문 등), **Snowflake datacenter·worker 설정 외부화** (단계 2 후속) |
| 높음 | **댓글 무한 depth** — 테이블·트리 설계, CUD·목록 API (README 섹션 3 항목 33~35) |
| 중간 | **파일**: 객체 스토리지(S3 호환), 게시글 삭제 시 **고아 파일 정리**, 다운로드 `Content-Disposition`·권한 세분화 (단계 3 후속) |
| 중간 | **좋아요·집계**: 고동시·Redis 카운터 튜닝, **사용자별 게시글·댓글 수** API (로드맵 41~43, 현재 미구현) |
| 중간 | **조회수**: 동일 사용자·IP·시간 윈도우 등 **어뷰징 방지** 설계·구현 (단계 6 후속) |
| 중간 | **인기·Kafka**: 토픽 파티션·DLQ·재처리, 인기 점수 가중치(좋아요 등), 브로커 미가용 **폴백** (단계 7 후속) |

### 문서·아키텍처 (코드 최소)

- [ ] **단계 1**: Monolith vs MSA 역할 정리 메모, 이후 Kafka·Redis·CQRS 도입 시 **Compose 서비스 확장 계획**

### 로드맵 깊은 쪽 (섹션 7 — 게시글 조회 최적화)

README **61~76**: CQRS, 조회 전용 모델·이벤트, 조회수·목록 캐시, **Request Collapsing** 등. 인프라·이벤트 파이프 정리 후 묶어서 진행하는 것을 권장.

### 권장 진행 순서 (한 줄)

사용자 체감이 큰 **검색** 또는 **댓글 무한 depth** → **조회수 어뷰징 방지** → **파일 S3·고아 정리** → **Outbox·Kafka 보강** → **CQRS·목록 최적화**.
...
| 2026-04-15 | 게시글 목록: 오프셋 `page` 제거 → `GET /posts` 커서 API + 프론트 `useInfiniteQuery`·IntersectionObserver 무한 스크롤 |
| 2026-04-15 | 커서 코덱: `epochSecond:nano:postId` 인코딩(나노초 정밀도), 레거시 `millis:id` 디코드 호환 |
| 2026-04-15 | 문서에 **다음 할 일** 섹션 추가 (로드맵 후속·권장 순서 정리) |
| 2026-04-15 | **테스트 실행 방법** 섹션 정리(백엔드·프론트·E2E), README에서 `#project-testing` 링크 |
